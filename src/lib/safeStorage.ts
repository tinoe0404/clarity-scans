...
export function safeGet<T>(key: string, validator: (value: unknown) => value is T): T | null {
  ...
  try {
    ...
    logger.warn(`Failed to read from localStorage key "${key}"`, { error: (error as Error).message });
    return null;
  }
}
...
export function safeSet(key: string, value: unknown): boolean {
  ...
  try {
    ...
    logger.warn(`Failed to set localStorage key "${key}"`, { error: (error as Error).message });
    return false;
  }
}
...
export function safeDelete(key: string): void {
  ...
  try {
    ...
    logger.warn(`Failed to delete localStorage key "${key}"`, { error: (error as Error).message });
  }
}
